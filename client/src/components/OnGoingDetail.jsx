...
export default function OngoingDetail() {
  ...
  useEffect(() => {
    ...
    const fetchPageData = async () => {
      setLoading(true);
      try {
        const res = await fetch(`${apiBaseUrl}/otakudesu/ongoing?page=${page}`);
        const data = await res.json();
        ...
        console.error(`Error fetching page ${page} for ongoing anime:`, error);
        setAnimeList([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <main className="min-h-screen bg-black text-white pt-16 px-4 md:px-12">
      <h1 className="text-2xl md:text-3xl font-bold mb-6">Ongoing Anime</h1>
      {loading ? (
